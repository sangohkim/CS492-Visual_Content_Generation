...
def wide_lognormal_timesteps(bsz, T=1000, mu=-0.3, sigma=1.6, device="cuda"):
    # 1. log-normal 샘플링 (양수 값)
    x = torch.distributions.LogNormal(mu, sigma).sample((bsz,)).to(device)

    # 2. [0,1] 범위로 squash (monotonic; 안정적)
    u = x / (1 + x)

    # 3. [0, T-1] 범위의 timestep으로 변환
    t = (u * (T - 1)).long()
    return t
...
def main(args):
    ...
    print(f"[DEBUG] column_names: {column_names}")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                timesteps = wide_lognormal_timesteps(
                    bsz,
