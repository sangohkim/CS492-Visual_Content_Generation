...
# coding=utf-8
"""
Comic Generation Script using StoryDiffusion + DreamBooth LoRA
This script generates consistent comic images using StoryDiffusion's Consistent Self-Attention mechanism
and supports loading DreamBooth LoRA weights for custom character generation.
"""
...
from diffusers import StableDiffusionXLPipeline, DDIMScheduler, DiffusionPipeline
...
class SpatialAttnProcessor2_0(torch.nn.Module):
    ...
    def __call__(self, attn, hidden_states, encoder_hidden_states=None, attention_mask=None, temb=None):
        ...
        print(f"hidden_states shape: {hidden_states.shape}, cur_step: {cur_step}, write: {write}")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate comics using StoryDiffusion with DreamBooth LoRA support")
    # Model settings
    parser.add_argument("--model", type=str, default="SDXL",
                       choices=["SDXL", "RealVision", "Juggernaut", "Unstable"],
                       help="Model to use for generation")
    parser.add_argument("--lora_path", type=str, default=None,
                       help="Path to DreamBooth LoRA weights directory (checkpoint)")
    parser.add_argument("--use_safetensors", action="store_true", default=True,
                       help="Use safetensors format for LoRA weights")
    # Generation settings
    ...
    # Output settings
    parser.add_argument("--output_dir", type=str, default="./comic_outputs_dreambooth",
...
def main():
    ...
    width = args.width
    print("=" * 80)
    print("Comic Generation with StoryDiffusion + DreamBooth LoRA")
    print("=" * 80)
    ...
    if args.lora_path:
        print(f"DreamBooth LoRA: {args.lora_path}")
    else:
        print("DreamBooth LoRA: Disabled")
    print("=" * 80)
    ...
    unet = pipe.unet
    # Load DreamBooth LoRA weights if provided
    print("[2/6] Loading DreamBooth LoRA weights...")
    if args.lora_path:
        try:
            pipe.load_lora_weights(args.lora_path, use_safetensors=args.use_safetensors)
            print(f"✓ Loaded LoRA weights from {args.lora_path}")
    ...
    if len(real_prompts) > 0:
        ...
        for idx, real_prompt in enumerate(real_prompts):
            ...
            print(f"prompt: {real_prompt_styled}")
            image = pipe(
                real_prompt_styled,
                num_inference_steps=args.num_steps,
                guidance_scale=args.guidance_scale,
