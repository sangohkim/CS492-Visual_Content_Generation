...
# coding=utf-8
"""
Comic Generation Script using StoryDiffusion with Textual Inversion Support
This script generates consistent comic images using StoryDiffusion's Consistent Self-Attention mechanism
and supports loading trained textual inversion embeddings.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate comics using StoryDiffusion with Textual Inversion support")
    ...
    parser.add_argument("--learned_embeds_path", type=str, default=None,
                       help="Path to the learned embeddings file for text_encoder (learned_embeds.safetensors)")
    parser.add_argument("--learned_embeds_2_path", type=str, default=None,
                       help="Path to the second learned embeddings file for text_encoder_2 (learned_embeds_2.safetensors)")
    ...
    parser.add_argument("--token_1", type=str, default=None,
                       help="Optional: different token name for text_encoder. If not provided, uses placeholder_token")
    parser.add_argument("--token_2", type=str, default=None,
    ...
    # Output settings
    parser.add_argument("--output_dir", type=str, default="./comic_outputs_ti",
    ...
    # Validate textual inversion arguments
    ti_args = [args.learned_embeds_path, args.learned_embeds_2_path, args.placeholder_token]
    if any(arg is not None for arg in ti_args):
        if not all(arg is not None for arg in ti_args):
            parser.error("When using textual inversion, all three arguments must be provided: "
                        "--learned_embeds_path, --learned_embeds_2_path, and --placeholder_token")
    
    return args


def load_textual_inversion_embeddings(pipe, args):
    """Load textual inversion embeddings into the pipeline"""
    if args.learned_embeds_path is None:
        print("No textual inversion embeddings specified. Skipping...")
        return False
...
def main():
    ...
    print("=" * 80)
    print("Comic Generation with StoryDiffusion + Textual Inversion")
    print("=" * 80)
    ...
    if args.learned_embeds_path:
        print(f"Textual Inversion: Enabled")
        print(f"  - Placeholder Token: {args.placeholder_token}")
        print(f"  - Text Encoder 1: {args.learned_embeds_path}")
        print(f"  - Text Encoder 2: {args.learned_embeds_2_path}")
    else:
        print("Textual Inversion: Disabled")
    print("=" * 80)
    ...
    print("\n[1/6] Loading Stable Diffusion XL pipeline...")
    ...
    # Load textual inversion embeddings if provided
    print("[2/6] Loading Textual Inversion Embeddings...")
    if not load_textual_inversion_embeddings(pipe, args):
        if args.learned_embeds_path is not None:
            print("Failed to load textual inversion embeddings. Exiting...")
            return
    
    # Insert Consistent Self-Attention
    print("[3/6] Setting up Consistent Self-Attention...")
    ...
    print("[4/6] Applying VRAM optimizations...")
    ...
    print(f"\n[5/6] Generating {id_length} identity images...")
    ...
    if len(real_prompts) > 0:
        print(f"[6/6] Generating {len(real_prompts)} additional panel images...")
